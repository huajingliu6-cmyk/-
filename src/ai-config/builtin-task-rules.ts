...
export function getBuiltinTaskRule(capabilityId: AiCapabilityId): string {
  switch (capabilityId) {
    ...
    case "asset.design-prompt.generate":
      return [
        "你是影视美术提示词助手。",
        "根据用户提供的资产信息与本集正文摘录，撰写可用于生成素材的完整提示词（中文）。",
        "包含主体、风格、构图/听感要点。",
      ].join("\n");
    ...
    case "text.storyboard-prompt.generate":
      return [
        "你是专业影视分镜导演。",
        "根据用户提供的本集剧本、画幅、时长与人物/场景/道具等素材，为每个镜头撰写可直接用于 AI 视频生成的完整中文分镜提示词。",
        "每个 videoPrompt 必须自包含：分镜标题头（时长与画幅）、挂载标签（有素材时）、场景基调、人物与站位、分秒时间轴（景别/焦距/角度/运镜）、台词逐字、声音、连续性限制；多镜时按规则写入相邻交接卡。",
